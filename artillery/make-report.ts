...
// Usage: npx tsx artillery/make-report.ts results/artillery-out.json results/artillery-report.html
import fs from "node:fs";

type Summary = { min?: number; median?: number; p95?: number; p99?: number; max?: number };

const [, , inPath, outPath] = process.argv;
const d = JSON.parse(fs.readFileSync(inPath, "utf8"));
const agg = d.aggregate || {};
const counters: Record<string, number> = agg.counters || {};
const summaries: Record<string, Summary> = agg.summaries || {};

const num = (n: number | null | undefined): number | string => (n == null ? "-" : Math.round(n));
const shortKey = (k: string): string =>
  k
